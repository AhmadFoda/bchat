...
class BCAuthCrypto:
    """A class containing a number of handlers for the secp384r1 ECC
algorithm in Python"""
    curve = ec.SECP384R1()
    cipher_key_size = 32
    mac_size = 16
    ...
    def ITOSP(self, longint, length):
        """ITOSP, short for Integer-to-Octet-String Primitive, converts a
non-negative integer
        to an octet string of a specified length. This particular function is
defined in the
        PKCS #1 v2.1: RSA Cryptography Standard (June 14, 2002)
        https://www.cryptrec.go.jp/cryptrec_03_spec_cypherlist_files/PDF/
pkcs-1v2-12.pdf"""
    ...
    def KDFX963(self, inbyte_x, shared_data, key_length, hashfunct=sha256,
hash_len=32):
        """KDFX963 is a key derivation function (KDF) that takes as input
byte sequence inbyte_x
        and additional shared data shared_data and outputs a byte sequence
key of length
        key_length. This function is defined in ANSI-X9.63-KDF, and this
particular flavor of
    ...
    def decrypt(self, cipher_text_b64, private_key):
        """Decrypt takes input base64-encoded data input cipher_text_b64 and
private key
        ...
        K = binascii.unhexlify(self.KDFX963(shared_key, V,
self.cipher_key_size + self.mac_size))
        K1 = K[:self.cipher_key_size]
        K2 = K[self.cipher_key_size:]
        T = cipher[ephemeral_key_len:]
        enc_data = T[:len(T) - self.mac_size]
        tag = T[-self.mac_size:]
        decryptor = Cipher(algorithms.AES(K1), modes.GCM(K2, tag),
backend=self.backend).decryptor()
        plain_text = decryptor.update(enc_data) + decryptor.finalize()
        return plain_text
    def generate(self):
        """Generate a public/private key pair and return the byte array
verions of each in a tuple."""
    ...
    def private_key_from_strings(self, public_key_b64, private_key_b64):
        """Generates the private key object from the Base64-encoded public
and private key strings."""
        public_number =ec.EllipticCurvePublicNumbers.from_encoded_point(self.curve,
base64.b64decode(public_key_b64))
        ...
        return private_key
def generate_pair():
    """Generates a public/private key pair using the crypto library, and
returns them as
    base64 strings."""
...
def decrypt_auth_token(tokenFromPayload, public_key_str, private_key_str):
    """Retrive the auth token and decrypt it, in a way that does not specify
